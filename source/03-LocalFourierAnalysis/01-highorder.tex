...
We start with a system of equations ${\color{burgundy}\mathbf{A}}$ representing a Galerkin operator, such as in Equation \ref{eq:jacobian_form}.
...
The localization of Fourier modes in two dimensions is given by
\begin{equation}
...
\end{equation}
In general, we will omit the subscript indicating the dimension.
...
\begin{equation}
\tilde{{\color{burgundy}\mathbf{A}}} = \mathbf{Q}^T \left( {\color{burgundy}\mathbf{A}}_e \odot \left[ e^{\imath \sum_d \left( \mathbf{x}_i - \mathbf{x}_j \right) \mathbf{\theta} / \mathbf{h}} \right] \right) \mathbf{Q}
